// Ooga Orbit flight: the attached stack as one body over a round world. Gravity pulls to the world's centre, the air
// thins with height, stages burn and drop, the lean on the way up stays in the launch plane, and the pod alone turns
// freely on all three axes with lift, heat on its shield and hull, stress, and a leaf chute. Fixed step, no allocation.
(() => {
  "use strict";
  ...
  // The world: the islands float above a sea on top of a sphere, launches go down the launch plane (x = 0) toward +z
  const R = 3000, SEA = -40, CY = SEA - R, GM = G0 * R * R;
  // The air: density at the sea, its scale height; where the sky turns black; the Sky Top, the height a flight is
  // going for, where Ooga NASA's sky hook holds the pod on a circle for the spacewalk
  const RHO0 = 0.003, SCALE_H = 30, SPACE_ALT = 140, ORBIT_ALT = 500;
  // Leaning on the way up: stick and gimbal, the Ooga's own lean with the engine out, damping, and how hard the air turns
  // the stack toward (fins) or away from (no fins) its flight path
  const LEAN = { gimbal: 1.3, coast: 0.35, damp: 2.2, air: 0.06 };
  const THROTTLE_RATE = 0.9;
  // Stress: dynamic pressure the stack takes head on, and how much a lean into the wind multiplies it
  const Q_LIMIT = 150, BEND = 5, OVER_T = 0.22;
  // Heat: flux per density and speed cubed, cooling, what the cone's point takes on the way up
  const HEAT_K = 3.4e-4, COOL = 0.35, ASCENT_EXPOSE = 0.15, SHIELD_BURN = 1e-4;
  // The pod: body rates on the three axes, how fast it follows the stick, the weathervane that turns the shield into
  // the wind, lift at an angle, and the drag of its blunt face
  const POD = { pitch: 1.5, roll: 2, yaw: 1.5, response: 6, weathervane: 0.012, lift: 0.34, cd: 1 };
  // The leaf chute glides like the drop's canopy: the highest and fastest it can open, bloom time, glide and sink
  // speeds, how fast the pod settles into them, the turn rate
  const CHUTE = { alt: 135, speed: 34, bloom: 1.6, forward: 5, sink: 4, grip: 1.6, turn: 0.9 };
  // Touchdowns: soft, hard and broken speeds on land and water
  const LAND = { soft: 8, hard: 14 }, SPLASH = { soft: 14, hard: 22 };
  // Heights above the sea under which the islands can be under a body
  const NEAR_GROUND = 90;
  // The air thins with height and is gone by the orbit line, so an orbit keeps for as long as the player likes
  ...
  const create = ({ stack, groundAt, onDrop = null }) => {
    ...
    // Where each part sits above the bottom of the whole stack
    ...
    // What is still attached: the lowest stage's index fixes the dry mass, the parked fuel above, the height and the air
    ...
    // The world's up at a point, and the launch plane's downrange direction there
    // and the cross-range direction, up × downrange
    ...
    // The lean, as a turn about the launch plane's normal from the world's up
    const setLean = () => {
    ...
    // Light the attached stage; `boost` scales its push for a moment after a clamp release that came early
    ...
    // Heading home: everything that can come off comes off (a pod with no knot under it keeps its last stage) and
    // the rest turns freely
    const homeward = () => {
      s.mode = "free";
    ...
    // Speed, height and the air where the body is now
    const measure = () => {
    ...
    // The sky hook: the body held still at a point; the attached stack eases upright by a fraction each call
    ...
    const touchdown = (water) => {
      ...
      // Coming down on a side is harder than on the bottom
    ...
    const substep = (dt, input) => {
      ...
      ay += s.up[1] * thrust / m;
      az += s.up[2] * thrust / m;
      // The angle between the flight and the body's up (or, for the pod, between the flight and its shield)
      ...
      if (s.mode === "free") {
        ...
        if (cosA > 0 && sinA > 1e-4 && s.chute !== "open") {
          ...
          // The body's up minus its part along the flight, normalised
        ...
        // Head on, and more drag and stress with a lean into the wind
      ...
      if (s.mode === "ascent" || s.mode === "pad") {
        localFrame(s.p);
        // The flight path's angle from the local up, and the lean against it
        ...
        if (s.chute === "open") {
          ...
          if (s.speed > 1) {
            // The shield's outward normal is -up; turn it toward the flight about (-up) × flight
