...
const headquarters = () => withPage("headquarters", hubPage(src), async (b) => {
  ...
  record("headquarters: distributed window daylight dims at night while the fixed warm-light pool stays bounded", lighting.sources === 21 && lighting.warm === 3 && lighting.daylight === 18 && lighting.unique === 21 && lighting.groups.every((group) => group.height && (group.kind === "panorama" ? group.count === 5 && group.spread > group.arc * 0.75 : group.count === 1)) && lighting.noon.count <= 10 && lighting.night.count <= 10 && lighting.noon.skyCount > 0 && lighting.night.skyCount > 0 && lighting.noon.fireCount > 0 && lighting.night.fireCount > 0 && lighting.night.sky < lighting.noon.sky * 0.25 && Math.abs(lighting.noon.fire - lighting.night.fire) < 0.00001 && lighting.stable, JSON.stringify(lighting));
