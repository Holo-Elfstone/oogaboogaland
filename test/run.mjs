...
const matrixNavigation = (backend) => withPage(`cave camera ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`${label}: low entry and exit preserve perspective through all five occupied apertures with and without the Matrix wave`, r.openings === 8 && r.occupied === 5 && r.cases.length === 10 && r.cases.every((c) => c.route.every((p) => p.requested[2] < 0.499 ? p.id === c.id && p.contains && Math.abs(p.actual[1] - 0.8) < 0.001 && Math.abs(p.actual[2] - p.requested[2]) < 0.001 && Math.abs(p.pitch) < 0.00001 : p.requested[2] > 0.501 ? p.index === 0 && (p.requested[2] < 1 ? Math.abs(p.actual[1] - 0.8) < 0.001 : Math.abs(p.actual[1] - Math.max(0.8, p.ground + 1.5)) < 0.001) : true)), JSON.stringify(routes));
...
const matrixHorizontal = (backend) => [`matrix horizontal lanes ${backend}`, async (b) => {
  ...
  record(`${label}: actual floor and ceiling instances move inward along unchanged lanes, including source-face seams`, r.motion.length === 48 && r.motion.every((m) => m.eligible > 3 && m.moved === m.eligible && m.gapPairs > 0 && m.distance >= 0.056 - 1e-8 && m.distance <= 0.12 + 1e-8 && m.error < 0.00001) && r.motion.reduce((sum, m) => sum + m.seams, 0) > 10, JSON.stringify(r.motion));
}];
