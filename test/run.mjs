...
import { roomMattressGeometryProbe, roomMattressVisibilityProbe, roomMattressMovementProbe, roomSleepProbe, roomManualSleepProbe } from "./room-mattresses.mjs";
...
import { cameraCoverProbe, cameraPartialCoverProbe, cameraRockTextureProbe, wallPanOutlineProbe, wholeWallMaskProbe } from "./camera-cover.mjs";
import { sleepOrientationProbe } from "./sleep-orientation.mjs";
import { rockGuidesProbe, sightGuideClippingProbe, sightGuidePerceptionProbe, objectSilhouetteProbe, objectPerceptionProbe, objectFadeProbe, sightGuideWorldProbe } from "./rock-guides.mjs";
...
const headquartersBareRoomProbe = () => {
  ...
  const bedding = new Set(H.mattresses.map((bed) => bed.node)), occupants = new Set();
  ...
  const extras = nodes.filter((node) => node.geometry && node.world[13] < -3 && !entrances.has(node.geometry) && !bedding.has(node) && !occupants.has(node) && rooms.some((room) => inside(node, room)));
...
const roomSleeping = (backend) => withPage(`room sleeping ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`room sleeping ${backend}: Randy starts asleep and all seven Oogas walk continuously to separate available beds`, r.initial.state === "sleeping" && r.initial.mode === "walk" && r.initial.visible && r.initial.claimed && r.startMotion < 1e-7 && r.claims && r.reserved === 7 && r.sleepSeconds < 180 && r.checks > 1000 && r.maxWalkStep <= 2 * r.dt + 1e-7 && r.failures.length === 0, JSON.stringify({ ...r, poses: undefined, returned: undefined }));
  record(`room sleeping ${backend}: every resting body lies on the unchanged printed mattress with slight compression and its head supported by the pillow`, r.poses.length === 7 && r.poses.every((p) => p.mode === "rest" && p.blocked === 0 && p.closed && p.pose === "left" && Math.abs(p.quaternionLength - 1) < 1e-6 && p.reserved && p.flatFabric && p.bodyInside && p.headClear && p.pillowContact > 1e-5 && Math.abs(p.pillowBottom - p.pillowTop + 0.015) < 1e-5 && p.headBottom >= p.surface - 0.02501 && Math.abs(p.headX) < 0.45 && Math.abs(p.headZ - p.pillowZ) < 0.25 && p.bodyFloor >= p.surface - 0.16001 && p.headOffset.every((offset) => Math.abs(offset) < 1e-6) && p.bodySamples > 1000), JSON.stringify(r.poses));
  record(`room sleeping ${backend}: waking releases each bed in place and follows a clear route back to its pile slot`, r.wake.movement < 1e-7 && r.wake.released && r.wake.returning && r.returned.length === 7 && r.returned.every((c) => c.mode === "" && c.state === "working" && c.y >= 0 && c.distance < 0.1 && !c.claimed) && r.failures.length === 0, JSON.stringify({ wake: r.wake, returned: r.returned, failures: r.failures }));
...
const headquarters = () => withPage("headquarters", hubPage(src), async (b) => {
  ...
  record("headquarters: sleepers reserve separate mattresses and waking releases every reservation without teleporting", sleepers.sleeping.length === 7 && sleepers.unique === 7 && sleepers.sleeping.every((sleeper) => sleeper.state === "sleeping" && sleeper.visible && sleeper.reserved && ["walk", "lie", "rest"].includes(sleeper.mode)) && sleepers.released && sleepers.awake.length === 7 && sleepers.awake.every((cave) => cave.state === "working" && cave.visible && !cave.claimedBed && cave.movement < 1e-7), JSON.stringify(sleepers));
...
for (const backend of ["webgl2", "canvas2d"]) task(`camera cover ${backend}`, () => withPage(`camera cover ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`camera cover ${backend}: crossing the floor reveals only the occupied near-plane rays with continuous partial coverage`, partial.rows.length === 33 && partial.rows[0].fraction === 0 && partial.rows.at(-1).fraction === 1 && partial.rows.filter((row) => row.partialRock && row.fraction > 0 && row.fraction < 1).length > 10 && partial.rows.every((row, i, rows) => !i || row.fraction >= rows[i - 1].fraction && row.fraction - rows[i - 1].fraction < 0.12) && partial.comparisons > 5000 && partial.failures.length === 0 && partial.screenshotCoverage > 0 && partial.screenshotCoverage < 1, JSON.stringify(partial));
...
for (const backend of ["webgl2", "canvas2d"]) task(`camera rock guides ${backend}`, () => withPage(`camera rock guides ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`camera rock guides ${backend}: occupied room, ramp and common-area references retain bounded cached structural geometry`, r.rows.length === 21 && r.rows.every((row) => row.cached && row.selected?.kind === row.kind && row.selected.index === row.index && row.selected.basement === row.basement && row.count > 0 && row.count <= 96) && r.sky && r.stats.contexts === 22 && r.stats.lines <= 21 * 96 && r.stats.taggedWindowReveals > 0 && r.stats.windowReveals === r.stats.taggedWindowReveals && r.finite && r.points > 5000 && r.failures.length === 0 && r.disposed, JSON.stringify({ ...r, views: undefined }));
...
for (const backend of ["webgl2", "canvas2d"]) task(`camera pile outline ${backend}`, () => withPage(`camera pile outline ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`camera pile outline ${backend}: the rock base, mound and settled bananas share one owner at every pile level`, r.rows.length === 6 && r.rows.map((row) => row.level).join(",") === "0,1,302,303,1000,10000000" && r.rows.every((row) => row.active && row.singleOwner && row.provider && row.instances === row.expectedInstances && row.registered === r.originalNodes && row.capacity === r.originalCapacity && row.buffers === r.bytes) && r.rows.at(-1).shell > 0 && r.dropChanged === 0 && r.failures.length === 0, JSON.stringify(r));
  record(`camera pile outline ${backend}: the full projected mesh union has a faint outer rim and no interior seams`, r.rows.every((row) => row.outline > 50 && row.maximumAlpha > 0 && row.maximumAlpha <= 46 && row.cached && (row.referencePixels === null || row.referencePixels > 300 && row.interior > 100 && row.seams === 0 && row.distant === 0)) && r.rows.filter((row) => row.referencePixels !== null).length >= 5 && new Set(r.rows.map((row) => row.fingerprint)).size > 3 && r.failures.length === 0, JSON.stringify(r.rows));
}));
for (const backend of ["webgl2", "canvas2d"]) task(`camera object registry ${backend}`, () => withPage(`camera object registry ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`camera object registry ${backend}: actual meadow grass remains rendered but never enters nearby owners or outlines`, grass.grass > 100 && grass.rendered && grass.excluded && grass.rows.length === 3 && grass.rows.every((row) => row.registered === 1 && row.nearby === 1 && row.controlLines > 0 && row.grassLines === 0 && !row.grassSources && !row.grassNearby && !row.productionSources && !row.productionNearby), JSON.stringify(grass));
