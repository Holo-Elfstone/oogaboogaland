...
import { matrixGateClipProbe } from "./matrix-gates.mjs";
...
const mirrorCave = ["mirror cave", async (b) => {
  ...
  await rendered(72);
  const gatesReset = await b.evaluate(`(() => { const B = window.__ooga, G = B.matrixGate; return { unlocked: G.unlocked, pressed: G.pressed, living: G.button.matrixLiving, glow: G.button.glow, buttonY: G.button.position.y, raised: G.gates.every((g) => !g.open && g.node.position.y === G.visibleHeight), portal: B.mirror.portal, reveal: B.mirror.reveal }; })()`);
  record("matrix gate control: pressing back out raises the bars from the ground and darkens the button", !gatesReset.unlocked && !gatesReset.pressed && !gatesReset.living && gatesReset.glow < 0.5 && gatesReset.buttonY === 1.12 && gatesReset.raised && gatesReset.portal && gatesReset.reveal === 1, JSON.stringify(gatesReset));
  await b.evaluate(`window.__ooga.matrixCave.viewApproach()`);
  await rendered(72);
...
const mirrorCanvas = () => withPage("mirror canvas fallback", hubPage(src, "canvas2d=1&bananas=1000000&hour=22"), async (b) => {
  const r = await b.evaluate(`(() => { const B = window.__ooga, C = B.mirrorCave, candidates = B.props.filter((o) => o.scenery); return { kind: B.renderer.kind, active: B.mirror.active, faux: B.mirror.faux, surfaceDrawn: B.mirror.surfaceDrawn, resources: B.mirror.resources, passes: B.mirror.reflectionPassCount, skipped: B.mirror.skippedPassCount, children: C.group.children.length, mirrorMarked: C.node.mirror === true, path: { active: B.path.active, inner: B.path.ringInnerRadius, outer: B.path.ringOuterRadius, count: B.path.visibleInstanceCount, capacity: B.path.bufferCapacity, masterMaskBuildCount: B.path.masterMaskBuildCount, masterMaskHash: B.path.masterMaskHash }, scenery: { ...B.scenery, signature: candidates.map((o) => [o.prop, o.x, o.z, o.node.rotation.y].join(":" )).join("|") } }; })()`);
  record("mirror canvas fallback: depth-sorted faux mirror draws without reflection resources", r.kind === "canvas2d" && r.active && r.faux && r.surfaceDrawn && r.resources === 0 && r.passes === 0 && r.skipped > 0 && r.children === 8 && r.mirrorMarked, JSON.stringify({ kind: r.kind, active: r.active, faux: r.faux, surfaceDrawn: r.surfaceDrawn, resources: r.resources, passes: r.passes, skipped: r.skipped, children: r.children, mirrorMarked: r.mirrorMarked }));
...
const matrixNavigation = (backend) => withPage(`cave camera ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  const interior = (p) => p.interior || p.requested[2] < 0.499;
  const routes = r.cases.map((c) => ({ id: c.id, active: c.active, samples: c.route.map((p) => ({ requested: p.requested, actual: p.actual, id: p.id, contains: p.contains, matrixInside: p.matrixInside, active: p.active })) }));
  record(`${label}: low entry and exit preserve perspective through all five occupied apertures with and without the Matrix wave`, r.openings === 8 && r.occupied === 5 && r.cases.length === 10 && r.cases.every((c) => c.route.every((p) => interior(p) ? p.id === c.id && p.contains && Math.abs(p.actual[0] - p.requested[0]) < 0.001 && Math.abs(p.actual[1] - p.requested[1]) < 0.001 && Math.abs(p.actual[2] - p.requested[2]) < 0.001 && Math.abs(p.pitch) < 0.00001 : p.requested[2] > 0.501 ? p.index === 0 && (p.requested[2] < 1 ? Math.abs(p.actual[1] - 0.8) < 0.001 : Math.abs(p.actual[1] - Math.max(0.8, p.ground + 1.5)) < 0.001) : true)), JSON.stringify(routes));
  record(`${label}: all three unused cave mouths are sealed and reject a direct low crossing`, r.sealed.length === 3 && r.sealed.map((s) => s.id).sort().join("|") === "c10|c2|c3" && r.sealed.every((s) => s.blocked && s.before.index === 0 && s.after.index === 0 && !s.after.contains && s.after.actual[2] >= 0.499), JSON.stringify(r.sealed));
  ...
  record(`${label}: only a valid Mirror Cave crossing changes its portal-inside state`, r.cases.every((c) => c.route.every((p) => interior(p) ? p.matrixInside === (c.id === "c1") : p.requested[2] > 0.501 ? !p.matrixInside : true) && (c.active ? c.route.every((p) => p.active) : c.id === "c1" ? c.route.find((p) => p.requested[2] === 0.4).active : c.route.every((p) => !p.active))), JSON.stringify(routes));
  ...
  record(`${label}: admitted cameras meet real walls and ceilings without being lofted onto the cliff`, r.cases.every((c) => c.ceiling.id === c.id && c.ceiling.contains && c.ceiling.ceiling !== null && c.ceiling.actual[1] <= c.ceiling.ceiling - 0.2999 && c.wall.id === c.id && c.wall.contains && (c.headquarters ? c.wall.blockedDistance > 0.5 : c.wall.actual[0] < c.wall.requested[0] - 0.5) && Math.abs(c.wall.actual[1] - c.wall.requested[1]) < 0.001), JSON.stringify(r.cases.map((c) => ({ id: c.id, active: c.active, ceiling: c.ceiling, wall: c.wall }))));
...
const matrixHorizontal = (backend) => [`matrix horizontal lanes ${backend}`, async (b) => {
  ...
  record(`${label}: smooth ramp glyphs tilt with their actual stone triangles and remain fully backed`, samples.every((s) => { const ramps = s.caves.filter((c) => c.slopeFaces); return ramps.length === 2 && ramps.every((c) => c.slopeFaces > 100 && c.slopeEscaped === 0 && c.slopeClearanceError < 0.00002); }) && r.before.caves.filter((c) => c.slopeFaces).every((c) => c.slopeCount + r.after.caves.find((next) => next.id === c.id).slopeCount > 0), JSON.stringify(samples.map((s) => s.caves.filter((c) => c.slopeFaces).map((c) => ({ id: c.id, faces: c.slopeFaces, count: c.slopeCount, escaped: c.slopeEscaped, clearanceError: c.slopeClearanceError })))));
...
const matrixRain = (backend) => withPage(`matrix falling rain ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  const permanent = r.inactive.caves.findIndex((c) => c.count > 0);
  record(`${label}: all eight caves restore native ceiling-to-floor rain without replacing their surface glyph sets`, r.backend === backend && r.drawn && full.every((s) => s.active && s.caves.length === 8 && s.caves.every((c) => c.streams === (backend === "canvas2d" ? 18 : 48) && c.count > 0 && c.count === c.reported && c.drawn === c.count && c.sharedStyle)), JSON.stringify(full.map((s) => s.caves.map((c) => ({ id: c.id, streams: c.streams, count: c.count, drawn: c.drawn, sharedStyle: c.sharedStyle })))));
...
const matrixWave = (backend) => [`matrix reversible wave ${backend}`, async (b) => {
  ...
  const gatesMatchFront = (sample) => sample.gates.items.length === 5 && sample.gates.items.every((gate) => gate.y === (sample.active && sample.radius >= gate.distance ? sample.gates.visible : sample.gates.hidden));
  const gateSamples = [r.start, r.mirrorWaiting, r.mirrorStarted, r.mirrorContinuing, r.mirrorGone, r.expanded, r.gatesRaised];
  record(`${label}: each occupied cave's bars appear immediately when the global glyph front reaches that entrance`, gateSamples.every(gatesMatchFront) && r.start.gates.items.every((gate) => gate.y === r.start.gates.hidden) && r.gatesRaised.gates.items.every((gate) => gate.y === r.gatesRaised.gates.visible) && r.mirrorGone.gates.items.find((gate) => gate.cave === r.start.gates.items.find((gate) => gate.distance === Math.max(...r.start.gates.items.map((v) => v.distance))).cave).y === r.mirrorGone.gates.visible, JSON.stringify(gateSamples.map((sample) => ({ radius: sample.radius, active: sample.active, gates: sample.gates }))));
...
const headquartersWindowProbe = () => {
  const B = window.__ooga, H = B.island.headquarters, geometry = B.island.geometry, triangles = [];
  for (const face of geometry.faces) if (face.headquartersRamp) for (let n = 1; n < face.i.length - 1; n++) {
  ...
  const inRamp = (x, y, z) => {
    for (const ramp of H.ramps) for (let i = 1; i < ramp.samples.length; i++) {
      ...
      if (ex * ex + ez * ez < (ramp.width / 2 - 0.1) ** 2 && y > floor + 0.1 && y < floor + 3.4) return true;
    }
    return false;
  };
  return H.windows.map((window) => {
    ...
    let clear = true, backed = true, stoneBelow = true, rampOpening = false, gallery = true, firstBlocked = null, firstRamp = null, firstGallery = null;
    for (const aperture of apertures) {
      ...
      let sill = false;
      for (let d = 0; d <= Math.min(aperture.length, 5); d += 0.125) if (B.island.solidAt(aperture.x + aperture.dx * d, window.sill - 0.25 + (window.rise || 0) * d, aperture.z + aperture.dz * d)) sill = true;
      stoneBelow &&= sill;
      for (const height of [0.25, window.height / 2, window.height - 0.25]) {
        ...
        for (let d = 0; d <= aperture.length; d += 0.125) {
          ...
          if (window.kind !== "ramp" && inRamp(x, floorY, z)) { rampOpening = true; firstRamp ||= [x, floorY, z]; }
        }
      }
    }
    const landingAngles = H.ramps.map((ramp) => Math.atan2(ramp.to.x, -ramp.to.z)).sort((a, b) => a - b);
    return { kind: window.kind, roomIndex: window.roomIndex, width: window.width, height: window.height, sill: window.sill, floor: window.floor, radius, outerRadius, arc: window.kind === "panorama" ? window.endAngle - window.startAngle : 0, betweenLandings: window.kind === "panorama" && window.startAngle > landingAngles[0] && window.endAngle < landingAngles[1], apertures: apertures.length, clear, meshClear: backed, stoneBelow, rampOpening, gallery, firstBlocked, firstRamp, firstGallery };
  });
};

const matrixGateClipping = () => withPage("matrix gate clipping", hubPage(src), async (b) => {
  const lifecycle = await b.evaluate(`(() => { const B = window.__ooga, G = B.matrixGate, scene = window.BL.scenes.hub, initial = G.gates.map((gate) => ({ hidden: !gate.node.visible && gate.node.position.y === G.hiddenHeight, floor: gate.node.geometry.clipMinY, mouthFloor: B.mouths.find((mouth) => mouth.id === B.matrixCave.caves.find((cave) => cave.caveIndex === gate.caveIndex).id).floorY })); let time = B.renderOpts.matrix.time; G.set(true); for (const gate of G.gates) gate.node.position.y = -1.7; scene.update(1 / 60, time += 1 / 60); const partial = G.gates.map((gate) => ({ visible: gate.node.visible, y: gate.node.position.y })); for (let i = 0; i < 40; i++) scene.update(1 / 60, time += 1 / 60); return { initial, partial, hiddenHeight: G.hiddenHeight, buried: G.gates.every((gate) => !gate.node.visible && gate.node.position.y === G.hiddenHeight) }; })()`);
  record("matrix gates: every cave clips at its own floor, keeps partial sinks visible, and suppresses fully buried nodes", lifecycle.initial.length === 5 && lifecycle.initial.every((gate) => gate.hidden && gate.floor === gate.mouthFloor) && lifecycle.partial.every((gate) => gate.visible && gate.y < -1.7 && gate.y > lifecycle.hiddenHeight) && lifecycle.buried, JSON.stringify(lifecycle));
  for (const backend of ["webgl2", "canvas2d"]) {
    const clipping = await b.evaluate(`(${matrixGateClipProbe.toString()})(${JSON.stringify(backend)})`);
    record(`matrix gates: ${backend} clips the actual sinking bars and glyph pixels at the world floor`, clipping.backend === backend && clipping.floor === 0 && clipping.parentY > 0 && clipping.samples.length === 8 && clipping.samples.every((s) => s.clipped.below === 0 && s.control.below > 0 && (s.sink < 3 ? s.clipped.above > 0 && Math.abs(s.control.above - s.clipped.above) <= 2 : s.clipped.above === 0)), JSON.stringify(clipping));
  }
});
...
const headquartersApronCameraProbe = () => {
  ...
  const pose = (m, across, y, along) => {
    ...
    return { id: m.id, across, along, y: p.y, ground: B.island.surfaceAt(x, z), drift: Math.hypot(p.x - x, p.z - z), index: B.cameraCave.index, lowerCeiling: B.island.cavityAt(x, z, lower, 9) ? lower.ceiling : null };
  };
  try {
...
const headquartersBareRoomProbe = () => {
  const B = window.__ooga, H = B.headquarters, M = window.BL.headquartersModels, scene = window.BL.scenes.hub, nodes = [];
  const walk = (node) => { nodes.push(node); for (const child of node.children) walk(child); };
  walk(scene.root);
  const entrances = new Set(H.rooms.map((room) => M.roomEntrance(room.index)));
  const inside = (node, room) => {
    const dx = node.position.x - room.x, dz = node.position.z - room.z;
    return Math.abs(dx * Math.cos(room.angle) + dz * Math.sin(room.angle)) < room.width / 2 && Math.abs(dx * Math.sin(room.angle) - dz * Math.cos(room.angle)) < room.depth / 2;
  };
  const extras = nodes.filter((node) => node.geometry && node.position.y < -3 && !entrances.has(node.geometry) && H.rooms.some((room) => inside(node, room)));
  ...
  try {
    for (const room of H.rooms) {
      ...
      for (const [side, depth, y] of [[room.width / 2 - 0.25, 0, -5.5], [-room.width / 2 + 0.25, 0, -5.5], [-0.85, 0.3, -6.8]]) {
        const x = room.x + cz * side + sx * depth, z = room.z + sx * side - cz * depth;
        B.camera.position = { x: room.x, y: -5.5, z: room.z };
  ...
  return { noBeds: !("cots" in H) && !("cot" in M), noWallAPI: !("personalWalls" in H) && !("cycleWall" in H), noNumberAPI: !("roomNumbers" in H), noDecorModels: !("wallBay" in M) && !("roomNumber" in M), numberNodes: nodes.filter((node) => node.geometry && "roomNumber" in node.geometry).length, extras: extras.map((node) => ({ x: node.position.x, y: node.position.y, z: node.position.z, faces: node.geometry.faces.length })), hits, savedThemes: localStorage.getItem("ooga-headquarters-walls-v1") };
};
...
const movementCollision = (backend) => withPage(`movement collision ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  cases.push({ id: "c5", mode: "trailing", dt: 1 / 20, fromNavigation: true });
  cases.push({ id: "c5", mode: "first-person", dt: 1 / 120, fromNavigation: true });
  ...
  for (const [i, fixture] of cases.entries()) {
    ...
    const r = await b.evaluate(`(${movementCollisionProbe.toString()})(${JSON.stringify(fixture)})`), name = `movement collision ${backend}: ${fixture.id} ${fixture.mode}${fixture.fromNavigation ? " after HQ navigation" : ""}`;
    const detail = { dt: r.dt, checkpoints: r.checkpoints, samples: r.samples, failed: r.failed, violations: r.violations, separation: r.maxSeparation, step: r.maxStep, stepAt: r.maxStep > 1 ? r.maxStepAt : null, final: r.final };
    record(`${name} follows continuous routes and exits without trapping or rock penetration`, r.backend === backend && r.scene === "hub" && r.completed && r.violations.length === 0 && r.maxSeparation < 6.5 && r.maxStep < 1, JSON.stringify(detail));
    if (r.underground) {
      const u = r.underground;
      record(`${name} horizontal movement stays on the lower elevation layer`, u.before.y < -5 && u.after.y < -5 && Math.hypot(u.after.x - u.before.x, u.after.z - u.before.z) > 0.35 && u.before.camera !== 0 && u.after.camera !== 0 && (fixture.mode !== "orbit" || u.targetBefore < 0 && u.targetAfter < 0), JSON.stringify(u));
    }
    if (r.collision) record(`${name} slides at wall contact and reverses immediately after sustained pressure`, r.collision.travel > 0.5 && r.reversal.firstDot > 0.0001 && r.reversal.distance > 0.25, JSON.stringify({ collision: r.collision, reversal: r.reversal }));
  }
});

const movementWindows = (backend) => withPage(`movement windows ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  for (const [i, fixture] of [{}, { roomIndex: 4 }].entries()) {
    if (i) { await b.open(hubPage(src, backend === "canvas2d" ? "canvas2d=1" : "")); await untilReady(b); }
    const r = await b.evaluate(`(${movementWindowsProbe.toString()})(${JSON.stringify(fixture)})`), label = r.kind === "room" ? "rear-room window" : "panorama";
    record(`movement windows ${backend}: free flight crosses the ${label} and returns to headquarters at the same height`, r.completed && r.backend === backend && r.scene === "hub" && r.violations.length === 0 && r.maxStep < 1 && r.admitted.layer === 9 && r.admitted.camera !== 0 && r.outside.radius > 30.8 && r.outside.camera === 0 && r.outside.layer === 0 && Math.abs(r.outside.y - r.expectedHeight) < 0.3 && r.reentered.layer === 9 && r.reentered.camera !== 0 && Math.abs(r.reentered.y - r.expectedHeight) < 0.3 && r.final.layer === 9 && Math.abs(r.final.y + 5.9) < 0.4 && !r.final.controlled && !r.final.matrixInside, JSON.stringify(r));
  }
});
...
const movementCeiling = (backend) => withPage(`movement ceiling ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  for (const [i, mode] of ["trailing", "first-person"].entries()) {
    ...
    record(`${name} the real posed head stops at the cave roof while thrust stays held`, !!c && c.equipped && c.entered.player !== 0 && c.hover.thrust && c.hover.hop > 0.5 && c.hover.headTop <= c.hover.roof + 0.01 && c.hover.roof - c.hover.headTop < 0.12 && c.plateauFrames >= Math.ceil(0.2 / r.dt) && c.meshSamples > 10000 && r.violations.length === 0, JSON.stringify({ contact: c, violations: r.violations }));
    record(`${name} sideways and diagonal thrust remain responsive and release lands before a continuous exit`, !!c && c.sideways > 0.4 && c.diagonalTravel > 0.4 && c.landingSeconds < 3 && c.landed.hop === 0 && !c.landed.thrust && Math.abs(c.landed.y) < 0.01 && r.completed && r.scene === "hub" && r.backend === backend && r.maxSeparation < 6.5 && r.maxStep < 1 && r.final.player === 0 && r.violations.length === 0, JSON.stringify({ side: c && c.sideways, diagonal: c && c.diagonalTravel, landing: c && c.landed, completed: r.completed, failed: r.failed, final: r.final, separation: r.maxSeparation, step: r.maxStep, violations: r.violations }));
  }
});
...
const rampCeiling = (backend) => withPage(`ramp ceiling ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  for (const [i, fixture] of cases.entries()) {
    ...
    record(`${name} descends with horizontal keys and ascends along the ceiling without sticking`, r.completed && r.descentClimbInputs === 0 && r.minimumY < -3 && r.roofContacts > 20 && r.maxStall < 0.5 && r.maxStep < 1.1 && r.violations.length === 0, JSON.stringify({ completed: r.completed, failures: r.failures, violations: r.violations, roofContacts: r.roofContacts, maxStall: r.maxStall, maxStep: r.maxStep, minimumY: r.minimumY, stages: r.stages }));
    const reverse = r.reversal;
    record(`${name} reverses on the upper curve and exits without recentring or changing mode`, !!reverse && Math.hypot(reverse.back.x - reverse.before.x, reverse.back.z - reverse.before.z) > 0.5 && Math.hypot(reverse.resumed.x - reverse.back.x, reverse.resumed.z - reverse.back.z) > 0.5 && r.completed && r.final.camera === 0 && r.final.mode === "orbit" && r.scene === "hub" && r.backend === backend && !r.selected, JSON.stringify({ reversal: reverse, final: r.final, scene: r.scene }));
  }
});
...
const navigationButtons = (backend) => withPage(`navigation buttons ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`navigation buttons ${backend}: the toolbar contains only PILE, LAB, MIRROR and HQ in that order`, layout.labels.join("|") === "PILE|LAB|MIRROR|HQ" && layout.buttons.map((button) => button.id).join("|") === "pile|lab|mirror|underground", JSON.stringify({ labels: layout.labels, ids: layout.buttons.map((button) => button.id) }));
  ...
  for (const [i, mode] of modes.entries()) {
    ...
    record(`${name} all destinations and round trips preserve the perspective, selected Ooga and equipment`, r.backend === backend && r.initial.mode === mode && r.initial.selected === (mode === "trailing" || mode === "first-person") && r.rows.every(preserves), JSON.stringify({ clicks: r.rows.length, initial: r.initial, failed: r.rows.filter((row) => !preserves(row)) }));
    ...
    if (r.airborne) record(`${name} an airborne arrival cancels thrust and falling speed without removing the jetpack`, r.airborne.before > 0.2 && r.airborne.after.hop === 0 && r.airborne.after.velocity === 0 && !r.airborne.after.thrust && r.airborne.after.sameEquipment, JSON.stringify(r.airborne));
...
const navigationButtonsMobile = () => withPage("navigation buttons mobile", hubPage(src, "canvas2d=1"), async (b) => {
  const layout = await b.evaluate(`(${navigationButtonLayoutProbe.toString()})()`), ids = ["pile", "lab", "mirror", "underground"];
  record("navigation buttons mobile: the centered clock and PILE, LAB, MIRROR and HQ controls remain clear and touchable", layout.labels.join("|") === "PILE|LAB|MIRROR|HQ" && layout.buttons.map((button) => button.id).join("|") === ids.join("|") && layout.resetCount === 0 && !layout.overlap && layout.brandClear && layout.clockCentered && layout.clockClear && layout.documentWidth <= layout.viewport && layout.buttons.length === ids.length && ids.every((id) => layout.buttons.some((button) => button.id === id && button.hittable && button.visible && button.width >= 44 && button.height >= 32 && button.x >= 0 && button.x + button.width <= layout.viewport)), JSON.stringify(layout));
  const tapped = [];
  for (const id of ["mirror", "underground", "lab", "pile"]) {
    const button = layout.buttons.find((entry) => entry.id === id);
    if (!button) continue;
    await b.send("Input.dispatchTouchEvent", { type: "touchStart", touchPoints: [{ x: button.x + button.width / 2, y: button.y + button.height / 2 }] });
    await b.send("Input.dispatchTouchEvent", { type: "touchEnd", touchPoints: [] });
    await untilPage(b, id === "underground" ? "B.cameraCave.index !== 0 && B.camera.position.y < 0" : "B.cameraCave.index === 0 && B.camera.position.y >= 0");
    tapped.push(await b.evaluate(`(() => { const B = window.__ooga; return { id: ${JSON.stringify(id)}, scene: B.scene, mode: B.pilot.mode, selected: !!B.pilot.player, y: B.camera.position.y, camera: B.cameraCave.index }; })()`));
  }
  record("navigation buttons mobile: real taps navigate underground and back without changing orbit mode", tapped.length === ids.length && tapped.every((row) => row.scene === "hub" && row.mode === "orbit" && !row.selected && (row.id === "underground" ? row.y < 0 && row.camera !== 0 : row.y >= 0 && row.camera === 0)), JSON.stringify(tapped));
}, { w: 390, h: 844, mobile: true });
...
const headquarters = () => withPage("headquarters", hubPage(src), async (b) => {
  ...
  await untilReady(b);
  const built = await b.evaluate(`(() => { const B = window.__ooga, H = B.headquarters, column = {}; B.island.cavityAt(0, 0, column, 9); const slots = window.BL.caves.slots.filter((s) => s.status === "headquarters"); return { scene: B.scene, separateScene: !!window.BL.scenes.headquarters, rooms: H.rooms.map((r) => ({ index: r.index, angle: r.angle, radius: r.radius, window: r.window, nook: !!r.nook, resident: r.resident ?? null, approach: r.approach, x: r.x, z: r.z, width: r.width, depth: r.depth, entrance: r.entrance, back: r.back })), slots: slots.map((s) => ({ id: s.id, scene: s.scene, name: s.name })), floor: H.openFloor, column, caveBytes: B.island.cavityBytes }; })()`);
  const sleepers = await b.evaluate(`(() => { const B = window.__ooga, entries = [...B.cavemen.values()], overrides = entries.map((cave) => cave.override); try { for (const cave of entries) cave.override = "sleeping"; B.refreshStates(true); const sleeping = entries.map((cave) => ({ state: cave.state, visible: cave.root.visible, hiddenAnchor: cave.bedroll.hidden, roomIndex: cave.bedroll.roomIndex, x: cave.root.position.x, y: cave.root.position.y, z: cave.root.position.z })); for (const cave of entries) cave.override = "working"; B.refreshStates(true); return { sleeping, awake: entries.map((cave) => ({ state: cave.state, visible: cave.root.visible, claimedBed: !!cave.bedroll, y: cave.root.position.y })) }; } finally { entries.forEach((cave, i) => { cave.override = overrides[i]; }); B.refreshStates(true); } })()`);
  record("headquarters: the lower floor remains embedded in the hub with no scene or cutscene boundary", built.scene === "hub" && !built.separateScene && built.slots.length === 2 && built.slots.map((s) => s.id).sort().join("|") === "c5|c730" && built.slots.every((s) => s.scene === null && s.name === "Headquarters") && built.column.caveIndex === 9 && built.column.floor === -7 && Math.abs(built.column.ceiling + 2.8) < 0.06 && built.caveBytes <= 262144, JSON.stringify({ scene: built.scene, slots: built.slots, floor: built.floor, column: built.column, caveBytes: built.caveBytes }));
  ...
  record("headquarters: all eleven caves have full flat floors and connected entrances while stone encloses the rear rooms and new gallery nooks", routes.rooms.length === 11 && routes.rooms.every((r) => r.connected && (r.nook ? r.walls.length === 3 && r.walls.every((wall) => wall.solid) : r.wall) && r.floorSamples >= (r.nook ? 143 : 81) && r.floorErrors.length === 0), JSON.stringify(routes.rooms));
  ...
  record("headquarters: numerous wide windows are real openings through the outer shell with raised stone sills", windows.length === 14 && windows.every((w) => w.width >= 2 && w.height >= 1 && w.sill > w.floor + 0.7 && w.clear && w.meshClear && w.stoneBelow && w.outerRadius >= 30), JSON.stringify(windows));
  const roomWindows = windows.filter((window) => window.kind === "room"), panorama = windows.filter((window) => window.kind === "panorama");
  record("headquarters: room windows look outside instead of opening into the ramp tunnels", roomWindows.length === 7 && built.rooms.every((room) => roomWindows.some((window) => window.roomIndex === room.index) === room.window) && built.rooms.filter((room) => !room.window).length === 4 && roomWindows.every((window) => !window.rampOpening), JSON.stringify(roomWindows));
  ...
  await b.evaluate(`(() => { const B = window.__ooga, m = B.mouths.find((mouth) => mouth.id === "c5"), cave = [...B.cavemen.values()].find((entry) => entry.state === "working" && !entry.walk && !entry.build), o = B.pilot.orbit; B.pilot.release(true); cave.root.position.x = m.apron.x; cave.root.position.z = m.apron.z; cave.root.position.y = cave.baseY + B.island.surfaceAt(m.apron.x, m.apron.z); cave.hop = cave.hopV = 0; cave.root.rotation.y = Math.atan2(m.x - m.apron.x, m.z - m.apron.z); B.pilot.possess(cave); o.yaw = o.tYaw = cave.root.rotation.y + Math.PI; o.pitch = o.tPitch = 0.25; o.dist = o.tDist = 3.5; })()`);
  ...
  await untilReady(b);
  const canvas = await b.evaluate(`(() => { const B = window.__ooga, H = B.headquarters, bare = (${headquartersBareRoomProbe.toString()})(); return { scene: B.scene, renderer: B.renderer.kind, frames: B.renderedFrames, rooms: H.rooms.length, ramps: H.ramps.length, bare }; })()`);
  record("headquarters: Canvas fallback keeps all eleven embedded rooms bare and old saved data untouched", canvas.scene === "hub" && canvas.renderer === "canvas2d" && canvas.frames >= 2 && canvas.rooms === 11 && canvas.ramps === 2 && canvas.bare.noBeds && canvas.bare.noWallAPI && canvas.bare.noNumberAPI && canvas.bare.noDecorModels && canvas.bare.numberNodes === 0 && canvas.bare.extras.length === 0 && canvas.bare.hits.length === 33 && canvas.bare.hits.every((kind) => !kind || !kind.startsWith("headquarters-")) && canvas.bare.savedThemes === preservedThemes, JSON.stringify(canvas));
});
...
const hubPerspective = () => withPage("hub perspective", hubPage(src), async (b) => {
  ...
  const cameraState = () => b.evaluate(`(() => { const B = window.__ooga, c = B.camera; return { mode: B.pilot.mode, mix: B.pilot.closeMix, p: [c.position.x, c.position.y, c.position.z], target: [c.target.x, c.target.y, c.target.z], floor: B.island.surfaceAt(c.position.x, c.position.z), yaw: B.pilot.orbit.yaw, pitch: B.pilot.orbit.pitch, dist: B.pilot.orbit.dist, near: c.near }; })()`);
  ...
  await wheel(1, 60);
  await settle("orbit", 0);
  const picked = await b.evaluate(`(() => { const B = window.__ooga, cave = [...B.cavemen.values()].find((v) => v.state === "working" && !v.walk && !v.build), o = B.pilot.orbit; B.pilot.possess(cave); o.tDist = o.dist = 10; o.tPitch = o.pitch = 0.75; return cave.traits.name; })()`);
  const zoomTrack = [];
  for (let i = 0; i < 7; i++) {
    await wheel(1, -60);
    await b.sleep(40);
  ...
  const trailingTrack = zoomTrack.filter((sample) => sample.mode === "trailing"), zoomDirect = zoomTrack.every((sample) => Math.abs(sample.dist - sample.targetDist) < 0.01), flattening = trailingTrack.length >= 5 && trailingTrack.slice(3).every((sample, i) => i === 0 || sample.viewPitch <= trailingTrack[i + 2].viewPitch + 0.000001);
  record("hub perspective: controlled zoom responds directly, flattens toward eye level, and enters first person at minimum distance", zoomDirect && flattening && trailingTrack[0].viewPitch > 0.7 && trailingTrack[trailingTrack.length - 1].viewPitch < 0.1 && zoomTrack[zoomTrack.length - 1].mode === "first-person", JSON.stringify(zoomTrack));
  ...
  await b.evaluate(`(() => { const cave = window.__ooga.crew.player; cave.hopV = 3; })()`);
  await b.sleep(150);
  const airborne = await b.evaluate(`(() => { const B = window.__ooga, cave = B.crew.player, eye = cave.root.position.y - cave.baseY + cave.headOffset * 0.95; return { hop: cave.hop, lift: cave.viewLift, groundLift: B.pilot.groundLift, eyeError: Math.abs(B.camera.position.y - eye) }; })()`);
  await b.evaluate(`(() => { const B = window.__ooga, cave = B.crew.player; cave.hop = 1.2; cave.hopV = -3; cave.root.position.y = cave.baseY + B.island.surfaceAt(cave.root.position.x, cave.root.position.z) + cave.hop; })()`);
  await b.sleep(100);
  const falling = await b.evaluate(`(() => { const B = window.__ooga, cave = B.crew.player, eye = cave.root.position.y - cave.baseY + cave.headOffset * 0.95; return { hop: cave.hop, velocity: cave.hopV, lift: cave.viewLift, groundLift: B.pilot.groundLift, eyeError: Math.abs(B.camera.position.y - eye) }; })()`);
  ...
  const traverseTrailing = async (limit, comparison) => {
    await b.send("Input.dispatchKeyEvent", { type: "keyDown", key: "w", text: "w" });
    const result = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, cave = B.crew.player, leg = cave.parts.legL, start = performance.now(), levels = new Set(), done = (z) => z ${comparison} ${limit}, state = { samples: 0, levels: 0, maxRootStep: 0, maxVisualStep: 0, maxCameraStep: 0, maxCameraAt: null, maxFootError: 0, easedBodyBoundaries: 0, easedCameraBoundaries: 0, liftSamples: 0 }, tick = () => { const p = cave.root.position, ground = p.y - cave.baseY, visual = p.y + cave.viewLift, foot = p.y + leg.position.y - cave.baseY * leg.scale.y, previous = state.previousGround; levels.add(ground); state.samples++; state.maxFootError = Math.max(state.maxFootError, Math.abs(foot - ground)); if (Math.abs(cave.viewLift) > 0.005) state.liftSamples++; if (previous !== undefined) { const rootStep = Math.abs(ground - previous), visualStep = Math.abs(visual - state.previousVisual), cameraStep = Math.abs(B.camera.position.y - state.previousCamera); state.maxRootStep = Math.max(state.maxRootStep, rootStep); state.maxVisualStep = Math.max(state.maxVisualStep, visualStep); if (cameraStep > state.maxCameraStep) { state.maxCameraStep = cameraStep; state.maxCameraAt = { z: p.z, ground, visual, lift: cave.viewLift, cameraY: B.camera.position.y, previousCamera: state.previousCamera, cave: B.cameraCave.index, mode: B.pilot.mode }; } if (rootStep > 0.1 && visualStep < rootStep * 0.8) state.easedBodyBoundaries++; if (rootStep > 0.1 && cameraStep < rootStep * 0.8) state.easedCameraBoundaries++; } state.previousGround = ground; state.previousVisual = visual; state.previousCamera = B.camera.position.y; if (done(p.z) || performance.now() - start > 3500) { state.levels = levels.size; state.z = p.z; delete state.previousGround; delete state.previousVisual; delete state.previousCamera; resolve(state); } else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
    await b.send("Input.dispatchKeyEvent", { type: "keyUp", key: "w" });
    return result;
  };
  ...
  record("hub perspective: third-person character and camera ascend and descend the same steep voxel hillside without step snaps", trailingTerrain.mode === "trailing" && trailingAscent.levels >= 4 && trailingDescent.levels >= 4 && trailingAscent.easedBodyBoundaries >= 10 && trailingDescent.easedBodyBoundaries >= 10 && trailingAscent.easedCameraBoundaries >= 10 && trailingDescent.easedCameraBoundaries >= 10 && trailingAscent.maxVisualStep < 0.2 && trailingDescent.maxVisualStep < 0.2 && trailingAscent.maxCameraStep < 0.2 && trailingDescent.maxCameraStep < 0.2 && trailingAscent.liftSamples > 20 && trailingDescent.liftSamples > 20 && trailingAscent.maxFootError < 0.000001 && trailingDescent.maxFootError < 0.000001 && Math.abs(trailingTerrain.footL - trailingTerrain.ground) < 0.000001 && Math.abs(trailingTerrain.footR - trailingTerrain.ground) < 0.000001, JSON.stringify({ ascent: trailingAscent, descent: trailingDescent, terrain: trailingTerrain }));
  ...
  await b.send("Input.dispatchKeyEvent", { type: "keyUp", key: "s" });
  record("hub perspective: first-person eye anchoring survives a low Mirror Cave entry and exit", caveEntered.scene === "hub" && caveEntered.playerIndex === caveEntered.expected && caveEntered.cameraIndex === caveEntered.expected && caveEntered.eyeError < 0.001 && caveExited.scene === "hub" && caveExited.playerIndex === 0 && caveExited.cameraIndex === 0 && caveExited.eyeError < 0.001, JSON.stringify({ caveEntered, caveExited }));
  ...
  await b.send("Input.dispatchKeyEvent", { type: "keyDown", key: "w", text: "w" });
  const entered = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, start = performance.now(), tick = () => { if (B.scene === "race" || performance.now() - start > 6000) resolve({ scene: B.scene, ms: Math.round(performance.now() - start) }); else requestAnimationFrame(tick); }; tick(); })`);
  await b.send("Input.dispatchKeyEvent", { type: "keyUp", key: "w" });
  record("hub perspective: first person stays low and passes smoothly through a valid cave entrance", caveView.mode === "first-person" && caveView.cameraY < 3 && entered.scene === "race", JSON.stringify({ caveView, entered }));
});
...
const hubTrailingCaveSplit = () => withPage("hub trailing cave split", hubPage(src), async (b) => {
  const result = await b.evaluate(`(() => {
    ...
    let elapsed = B.matrixCave.world.sampleStream(0).time;
    const place = (z) => { const p = cave.root.position; p.x = m.x + opening.sr * z; p.y = cave.baseY + m.floorY; p.z = m.z + opening.cr * z; };
    const sample = () => { const p = B.camera.position, dx = p.x - m.x, dz = p.z - m.z; return { x: dx * opening.cr - dz * opening.sr, y: p.y - m.floorY, z: dx * opening.sr + dz * opening.cr, player: B.cameraCave.playerIndex, camera: B.cameraCave.index, entrance: B.cameraCave.entranceIndex, constraint: B.cameraCave.constraint, yaw: o.yaw, targetYaw: o.tYaw }; };
    const step = () => { scene.update(dt, elapsed += dt); return sample(); };
    B.pilot.release(true); place(0.9); B.pilot.possess(cave); cave.hop = cave.hopV = 0; cave.root.rotation.y = m.ry + Math.PI;
    o.target = { x: cave.root.position.x, y: m.floorY + 1.5, z: cave.root.position.z }; o.tx = o.target.x; o.ty = o.target.y; o.tz = o.target.z;
    o.yaw = o.tYaw = m.ry; o.pitch = o.tPitch = 0.08; o.dist = o.tDist = 6;
    for (let i = 0; i < 8; i++) step();
    let previous = sample(), entryMaxYStep = 0;
    for (let i = 1; i <= 120; i++) { place(0.9 - 4.4 * i / 120); const next = step(); entryMaxYStep = Math.max(entryMaxYStep, Math.abs(next.y - previous.y)); previous = next; }
    ...
    return { cave: opening.caveIndex, entryMaxYStep, split, left: orbit(105), right: orbit(-210), terrain: { verts: B.island.geometry.verts.length, faces: B.island.geometry.faces.length } };
  })()`);
  const stable = (turn) => turn.immediateMove > 1 && turn.immediateYawError < 0.000001 && turn.postInputDrift < 0.01 && turn.cameraChanges === 0 && turn.playerChanges === 0 && turn.idleDrift < 0.01 && turn.yawError < 0.001;
  record("hub trailing cave split: the Ooga and camera retain independent cave admission while orbiting across the mouth", result.split.player === result.cave && result.split.camera === 0 && result.split.entrance === result.cave && result.split.constraint === "entrance" && result.split.z > 0.5 && stable(result.left) && stable(result.right), JSON.stringify(result));
  record("hub trailing cave split: movement tracking stays vertically smooth while manual orbit repositioning is immediate", result.entryMaxYStep <= 3.2 / 120 + 0.0001 && result.left.immediateMove > 1 && result.right.immediateMove > 1 && result.left.postInputDrift < 0.01 && result.right.postInputDrift < 0.01, JSON.stringify({ entry: result.entryMaxYStep, left: { move: result.left.immediateMove, yaw: result.left.immediateYawError, drift: result.left.postInputDrift }, right: { move: result.right.immediateMove, yaw: result.right.immediateYawError, drift: result.right.postInputDrift } }));
  const state = () => b.evaluate(`(() => { const B = window.__ooga, o = B.pilot.orbit, p = B.camera.position; return { player: B.cameraCave.playerIndex, camera: B.cameraCave.index, yaw: o.yaw, targetYaw: o.tYaw, position: [p.x, p.y, p.z] }; })()`);
...
const hubPerspectiveTouch = () => withPage("hub perspective touch", hubPage(src), async (b) => {
  ...
  await b.evaluate(`(() => { const o = window.__ooga.pilot.orbit; o.tDist = o.dist = 3.5; })()`);
...
const hubDrive = () => withPage("hub drive", hubPage(src), async (b) => {
  ...
  record("hub drive: Space acts", stagedAct && ((act.hop && act.bubbles >= 1) || act.toast.length > 0), JSON.stringify({ stagedAct, ...act }));
...
const bannerClock = () => withPage("banner clock", `${src}?debug=1&nosim=1`, async (b) => {
  const local = await b.evaluate(`(() => { const el = document.getElementById("world-clock"), title = document.querySelector(".brand h1"), now = new Date(), [h, m] = el.dateTime.split(":").map(Number), current = now.getHours() * 60 + now.getMinutes(), shown = h * 60 + m, delta = Math.min(Math.abs(current - shown), 1440 - Math.abs(current - shown)), svg = el.querySelector("svg"), path = el.querySelector("path"); return { delta, label: el.getAttribute("aria-label"), dateTime: el.dateTime, viewBox: svg.getAttribute("viewBox"), glyphs: path.getAttribute("d").length, fontSize: parseFloat(getComputedStyle(el).fontSize), titleSize: parseFloat(getComputedStyle(title).fontSize), rect: el.getBoundingClientRect().toJSON() }; })()`);
  record("banner clock: normal mode shows the computer's local time in title-sized Ooga Booga block lettering", local.delta <= 1 && local.label.startsWith("Local time ") && /^\d\d:\d\d$/.test(local.dateTime) && local.viewBox === "0 0 30 6" && local.glyphs > 100 && local.rect.width > 0 && local.fontSize === local.titleSize, JSON.stringify(local));
...
const preloadedDebugView = () => withPage("preloaded debug view", hubPage(src, "view=hq"), async (b) => {
  ...
  await b.open(`${src}?nosim=1&view=hq`);
  const ignored = await b.evaluate(`new Promise((resolve) => { let frames = 0; const tick = () => { if (++frames < 6) requestAnimationFrame(tick); else { const scene = window.BL.scenes.hub, p = scene.camera.position; resolve({ debug: !!window.__ooga, y: p.y, targetY: scene.camera.target.y }); } }; requestAnimationFrame(tick); })`);
  record("preloaded debug view: view is ignored outside debug mode", !ignored.debug && ignored.y > 0 && ignored.targetY >= 0, JSON.stringify(ignored));
});
