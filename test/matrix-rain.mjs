...
export const matrixRainProbe = (prime = () => {}) => {
  ...
  const rainSets = [...C.caves.map((c) => c.rain), C.gateRain];
  const buffers = rainSets.flatMap((rain) => rain.nodes.map((n) => n.instanceData));
  ...
  const gateSnapshot = () => {
    const rain = C.gateRain;
    let count = 0, finite = true, upright = true, twoSided = true, within = true, leaders = 0, second = 0, trailing = 0, minGlow = Infinity, maxGlow = 0;
    for (const node of rain.nodes) for (let i = 0; i < node.instanceCount; i++) {
      ...
      within &&= x >= rain.minX - 0.00001 && x <= rain.maxX + 0.00001 && y >= rain.minY - 0.00001 && y <= rain.maxY + 0.00001 && z >= rain.minZ - 0.00001 && z <= rain.maxZ + 0.00001;
      if (tip > 0.99) leaders++; else if (tip > 0) second++; else trailing++;
      minGlow = Math.min(minGlow, glow); maxGlow = Math.max(maxGlow, glow);
    }
    return { streams: rain.streams.length, count, drawn: rain.nodes.reduce((n, node) => n + node.drawInstanceCount, 0), reported: rain.activeGlyphCount, updates: rain.updates,
      heads: rain.streams.map((stream) => stream.maxY - mod(elapsed * stream.speed + stream.phase, stream.period)), speeds: rain.streams.map((stream) => stream.speed), periods: rain.streams.map((stream) => stream.period),
      capacity: rain.capacity, bytes: rain.bufferBytes, buffers: rain.nodes.length, fixed: rain.nodes.every((node) => node.fixedInstanceCapacity && node.instanceCount <= rain.perGlyphCapacity),
      finite, upright, twoSided, within, leaders, second, trailing, minGlow, maxGlow };
  };
  const capture = (inspect = false) => ({ time: elapsed, radius: W.radius, active: W.active, direction: W.direction, inside: C.inside, records: R.stats.records, gate: gateSnapshot(),
    clouds: { count: C.clouds.length, wrapped: C.clouds.filter((cloud) => cloud.node.matrixCloud && !cloud.node.matrixLiving && !cloud.node.matrixEmissiveLiving).length,
  ...
  advance(20); step(true); const receded = capture();
  advance(0); step(true); const restored = capture();
  for (let i = 0; i < 12; i++) step(); const idle = capture();
  const current = rainSets.flatMap((rain) => rain.nodes.map((n) => n.instanceData));
  const compact = (s) => ({ ...s, caves: s.caves.map(({ rows, ...c }) => c) });
  return { backend: R.kind, drawn: R.ready, buffersStable: buffers.every((buffer, i) => buffer === current[i]), totalBytes: buffers.reduce((n, b) => n + b.byteLength, 0), inactive: compact(inactive), reversed: compact(reversed), before: compact(before), after: compact(after), motion, gateMotion, tiers: tiers.map(compact), receded: compact(receded), restored: compact(restored), idle: compact(idle) };
};
